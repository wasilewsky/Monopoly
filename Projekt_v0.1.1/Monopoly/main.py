...
class Message:
    ...
    def add_message(self, message): # ogarnąc żeby zapisywało do 10 ostatnich wiadomości
    ...
    def add_house(self):
        ...
        if not self.houses == 4:
            ...
            for x in down:
                if self.name == x:
                    pass
                    # pygame.draw.rect(screen, 'WHITE', (self.pos_x + 40, self.pos_y + 40 , 15, 15))
...
class Board:
    def __init__(self, x, y, screen):
        ...
        self.dane = (
            ...
            gm.POLEKASADOL,
            gm.POLEBROWN,
            gm.POLETAXDOL,        # =========== zrobić grafikę TAXDOL poleTaxDol.png
            gm.POLESATELITAS,
            gm.POLELIGHTBLUE,
            gm.POLESZANSADOL,
            gm.POLELIGHTBLUE,
            ...
            gm.POLEYELLOW,
            gm.POLEYELLOW,
            gm.POLECRYPTO,  # zmienione POLECLOUD
            gm.POLEYELLOW,
            gm.POLEGOTOJAIL,
            gm.POLEGREEN,
            gm.POLEGREEN,
...
def game(color):
    ...
    kup_button = Button(1220, 800, gm.KUP)
    ...
    game_board.add_players_to_field(players)  # każde pole ma dostęp do graczy - potrzebne do więzienia
    ...
    while window_open:
        ...
        if rzut_button.click():
            turn.double_roll()
            message.add_message(f"gracz {players[0].name} poruszył się o {turn.show_turn_value()}")
        ...
        for field in game_board.fields:
            ...
            if field.click() and field.typ == 'POLE':
                ...
                pygame.draw.rect(screen, (0, 0, 0), (pos_x - 7, pos_y - 7, 164, 54), 2, 10, -10, -10, -10,
                                 -10)  # ramka koloru
                text5 = pygame.font.Font.render(pygame.font.SysFont(None, 25), f"{field.nazwa}", True, (0, 0, 0))
                screen.blit(text5, (pos_x, (pos_y + 12)))
                # ======================
                for att in gm.data[0]:
                    if att not in ['typ', 'cena', 'kolor', 'nazwa', 'hipoteka', 'splata hipoteki']:
                        if hasattr(field, att):
                            text3 = pygame.font.Font.render(pygame.font.SysFont(None, 20), f"{att}: ", True, (0, 0, 0))
                            text4 = pygame.font.Font.render(pygame.font.SysFont(None, 20), f"${getattr(field, att)}",
                                                            True, (0, 0, 0))
