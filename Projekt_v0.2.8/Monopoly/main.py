...
class Player:
    def __init__(self, name, color, money, message):
        ...
        self.chance_card_picked = False
        self.show_card = False
        self.card = None
        self.display_start_time_card = 0
        self.card_picked = False
        # do wyswietlania kart po zakonczeniu ruchu ale nie dziala xd
        self.moved = False
        # --------
        ...
        if self.purple:
            if self.name == '11' and self.players[player_index[3]].prison:
                ...
                pygame.draw.rect(screen, 'PURPLE', (
                    self.pos_x + self.image.get_width() / 4 + 20, self.pos_y + self.image.get_height() / 4 + 20, 15,
                    15))
...
class System:
    ...
    def set_chance_card(self):
        for player in self.players:
            # czy gracz stoi na polu Szansy
            if (
                    player.position == 8 or player.position == 23 or player.position == 37) and player.moving == False:
                player.chance_card = self.random_chance()
                player.show_chance_card = True
                player.display_start_time_chance = pygame.time.get_ticks()
                pygame.time.delay(500)
                self.chance_card_action(player)
                break

            # czy gracz stoi na polu Kasy Spolecznej
            elif (
                    player.position == 3 or player.position == 18 or player.position == 34) and player.moving == False:
                player.card = self.random_card()
                player.show_card = True
                player.display_start_time_card = pygame.time.get_ticks()
                pygame.time.delay(500)
                self.card_action(player)
                break

    def display_chance_card(self):
        for player in self.players:
            if player.show_chance_card:
                pos_y = 570
                pos_x = 400
                text2 = pygame.font.Font.render(pygame.font.SysFont(None, 20), f"{player.chance_card[1]}", True,
                                                (0, 0, 0))
                ...
                # czy minął czas wyświetlania
                current_time = pygame.time.get_ticks()
                if current_time - player.display_start_time_chance >= 4000:
                    player.show_chance_card = False
                    pygame.time.delay(500)
    ...
    def display_card(self):
        for player in self.players:
            if player.show_card:
                pos_y = 570
                pos_x = 400
                text2 = pygame.font.Font.render(pygame.font.SysFont(None, 20), f"{player.card[1]}", True, (0, 0, 0))
                ...
                # czy minął czas wyświetlania
                current_time = pygame.time.get_ticks()
                if current_time - player.display_start_time_card >= 4000:
                    player.show_card = False
                    pygame.time.delay(500)

    # DZIALANIE KART SZANS
    def chance_card_action(self, player):
        card_number, card_description = player.chance_card
    ...
    # DZIALANIE KART KASY SPOLECZNEJ
    def card_action(self, player):
        card_number, card_description = player.card
        ...
        if card_number == 1:        # ZA KAZDY DOM $40 A ZA HOTEL $115
            ...
            self.players[1].money -= 10
            self.players[2].money -= 10
            self.players[3].money -= 10
            player.money += 40
        elif card_number == 7:
            player.move(0)
            player.money += 100
    ...
    def ai_actions(self):
        if self.current_player_index != 0:
            ...
            for x in range(0, self.dice.show_turn_value()):
                ...
                pygame.time.delay(200)
                self.draw_game(True, False)
            self.players[self.current_player_index].moving = False  # koniec ruchu ai
            ...
            if actual_field is not None:
                ...
                if actual_field.name in fields:
                    ...
                    while how_long > 0:
                        if actual_field.owner is not None and actual_field.owner.color is self.players[
                            self.current_player_index].color:
