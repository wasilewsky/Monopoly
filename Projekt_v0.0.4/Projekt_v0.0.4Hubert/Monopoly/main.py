...
# from Monopoly.Table import Table
from Monopoly.Table import Table, PlayerTable
    ...
    def create_fields(self):
        ...
        for x in range(0, 9):
            if x == 1 or x == 4 or x == 7:
                field = Field(self.pos_x + self.data[0][1] + (x * self.data[1][1]), self.pos_y, self.data[3][0],
                              f"{number}")
            else:
                field = Field(self.pos_x + self.data[0][1] + (x * self.data[1][1]), self.pos_y, self.data[1][0],
                              f"{number}")
            self.fields.append(field)
            number += 1
        ...
        for x in range(0, 9):
            if x == 2 or x == 4 or x == 7:
                field = Field(self.pos_x, self.pos_y + self.data[0][2] + (x * self.data[2][2]), self.data[4][0],
                              f"{number}")
            else:
                field = Field(self.pos_x, self.pos_y + self.data[0][2] + (x * self.data[2][2]), self.data[2][0],
                              f"{number}")
            self.fields.append(field)
            number -= 1
        ...
        for x in range(0, 9):
            if x == 2 or x == 4 or x == 5 or x == 7:
                field = Field(self.pos_x + self.data[0][1] + (x * self.data[1][1]),
                              self.pos_y + self.data[0][2] + 9 * self.data[2][2], self.data[3][0], f"{number}")
            else:
                field = Field(self.pos_x + self.data[0][1] + (x * self.data[1][1]),
                              self.pos_y + self.data[0][2] + 9 * self.data[2][2], self.data[1][0], f"{number}")
            self.fields.append(field)
            number -= 1

        # prawa dol
        field = Field(self.pos_x + self.data[0][1] + 9 * self.data[1][1],
                      self.pos_y + self.data[0][2] + 9 * self.data[2][2], self.data[0][0], f"1")
        self.fields.append(field)
        ...
        for x in range(0, 9):
            if x == 2 or x == 4 or x == 7:
                field = Field(self.pos_x + self.data[0][1] + 9 * self.data[1][1],
                              self.pos_y + self.data[0][2] + (x * self.data[2][2]), self.data[4][0], f"{number}")
            else:
                field = Field(self.pos_x + self.data[0][1] + 9 * self.data[1][1],
                              self.pos_y + self.data[0][2] + (x * self.data[2][2]), self.data[2][0], f"{number}")
            self.fields.append(field)
            number += 1
...
def game(color):
    ...
    while window_open:
        ...
        field = gra.get_field(PlayerTable.player1.get_position())
        field.set_player_vis('p1', True)

        if rzut.click():
            field.set_player_vis('p1', False)
            ruch.double_roll()
            PlayerTable.player1.move(ruch.show_turn_value())
        ...
        text = pygame.font.Font.render(pygame.font.SysFont(None, 72), f"rzut {color} kostką: {ruch.show_turn_value()}",
                                       True, (0, 0, 0))
        screen.blit(text, (220, 170))
        text2 = pygame.font.Font.render(pygame.font.SysFont(None, 72), f"dobra ale osobno: {ruch.dice1},{ruch.dice2}",
                                        True, (0, 0, 0))
...
def color():
    ...
    while window_open:
        ...
        if redBtn.click():
            PlayerTable.players[0].color = "red"
            PlayerTable.players[1].color = "blue"
            PlayerTable.players[2].color = "green"
            PlayerTable.players[3].color = "purple"
            window_open = False
            if not game("red"):
                window_open = True
        if blueBtn.click():
            PlayerTable.players[0].color = "blue"
            PlayerTable.players[1].color = "red"
            PlayerTable.players[2].color = "green"
            PlayerTable.players[3].color = "purple"
            window_open = False
            if not game("blue"):
                window_open = True
        if greenBtn.click():
            PlayerTable.players[0].color = "green"
            PlayerTable.players[1].color = "blue"
            PlayerTable.players[2].color = "red"
            PlayerTable.players[3].color = "purple"
            window_open = False
            if not game("green"):
                window_open = True
        if purpleBtn.click():
            PlayerTable.players[0].color = "purple"
            PlayerTable.players[1].color = "blue"
            PlayerTable.players[2].color = "green"
            PlayerTable.players[3].color = "red"
            window_open = False
            if not game("purple"):
                window_open = True
